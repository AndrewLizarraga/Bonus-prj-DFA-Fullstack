export function createAnimationController(renderer, options = {}) {
    ...
    function setAutomaton(automaton) {
        ...
        if (currentAutomaton) {
            renderer.render(currentAutomaton);
        }
    }

    function loadTrace(automaton, traceData) {
        currentAutomaton = automaton || null;
    ...
    function renderCurrentTransition() {
        ...
        renderer.render(currentAutomaton, highlight);
    ...
    async function play() {
        if (!currentAutomaton || currentSteps.length === 0) {
            return;
        }
