function PdaGridWalkCanvas({ result, activeStepIndex }) {
  ...
  function getMovementType(step) {
    ...
    if (step.stack_action?.includes("push") && !step.stack_action?.includes("push []")) {
      return "push";
    }
  ...
  function getMovementSymbol(movementType) {
    if (movementType === "push") return "↑";
    if (movementType === "pop") return "↓";
    if (movementType === "epsilon") return "ε";
    if (movementType === "initialize") return "•";
    return "→";
  }
  ...
  function buildGrid() {
    ...
    visibleSteps.forEach((step) => {
      const movementType = getMovementType(step);

      if (movementType === "push") {
        row = wrapPosition(row - 1);
      } else if (movementType === "pop") {
        row = wrapPosition(row + 1);
      ...
      grid[row][col] = {
        ...
        movementType,
        movementSymbol: getMovementSymbol(movementType),
      };
    });
